    ...
    <v-container>
      <v-col cols="10" style="margin:0 auto;">
        ...
        <v-row align="center">
          ...
          <v-col cols="10" sm="4">
            <v-select
              v-model="Adoptupdate.gugun"
              :items="gugun_states[Adoptupdate.sido]"
              hint="거주 지역을 선택하세요"
              persistent-hint
            ></v-select>
          </v-col>
...
export default {
  ...
  methods: {
    ...
    submitupdate() {
      ...
      axios.put(`http://localhost:8080/postscript/Modify?postscriptno=${this.$route.params.ID}&title=${this.Adoptupdate.title}&content=${this.Adoptupdate.content}&sido=${this.Adoptupdate.sido}&gugun=${this.Adoptupdate.gugun}&kind=${this.Adoptupdate.kind}`)
      .then(()=>{
