...
export default {
  ...
  methods: {
    ...
    adoptdetail(){
      axios.get(constants.SERVER_URL + "/postscript/detail?postscriptno="+this.$route.params.ID)
      .then((res) =>{
    ...
    postdelete(){
      ...
      axios.delete(constants.SERVER_URL + `/postscript/Delete?postscriptno=`+this.$route.params.ID)
      .then(()=>{
         this.adoptlist()
      })
      .catch((error)=>{
