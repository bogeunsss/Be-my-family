...
export default {
  created() {
    ...
    axios
      .get(constants.SERVER_URL + `/lost/detail?lostno=${this.lostno}`)
      .then((response) => {
        ...
        this.subContents.장소 =
          result.lostsido + " " + result.lostgugun + " " + result.lostplace;
    ...
    axios
      .get(constants.SERVER_URL + `/lost/match?lostno=${this.lostno}`)
      .then((response) => {
        // console.log(response)
  ...
  methods: {
      ...
      axios
        .delete(
          constants.SERVER_URL +
            `/lost/delete?lostno=${this.$route.params.articleNo}&uid=${this.writer}`
        )
        .then((response) => {
          alert("삭제되었습니다.");
    ...
    submit() {
      ...
      axios
        .post(constants.SERVER_URL + "/lost/add", formData, {
          headers: {
    ...
    commentCreate() {
      if (this.$cookies.isKey("auth-token")) {
        axios
          .post(constants.SERVER_URL + "/lost/reply/add", {
    ...
    commentDelete(replyno) {
      axios
        .delete(
          constants.SERVER_URL +
            `/lost/reply/delete?lostreplyno=${replyno}&uid=${this.profileData.nickName}`
        )
        .then((response) => {
          if (response.data.data === "success") {
            this.getCommentList();
