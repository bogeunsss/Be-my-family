      ...
      <form>
        <v-row class="d-flex align-center mx-auto">
          ...
          <v-col cols="8" sm="8" md="8">
            <v-text-field label="검색어를 입력하세요" v-model="searchText"></v-text-field>
          </v-col>
          <v-col class="mb-4 pa-0" cols="2" sm="2" md="2">
            <v-btn large @click="search" type="submit">검색</v-btn>
          ...
          <v-row>
            <v-col col="6" md="3">
              <v-card @click="goDetail">
                <v-img class="white--text align-end" height="200px" :src="dogData.popfile">
  ...
  methods: {
    ...
    goDetail() {
      this.$router.push({
        ...
        params: { desertionno: this.dogData.desertionno },
      });
    },
    search() {
      ...
      if (this.searchText === "") {
        ...
        axios
          .get(
            `http://localhost:8080/care/search?category=${this.category}&searchText=${this.searchText}`
          )
          .then((response) => {
